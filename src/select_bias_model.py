...
from utils.palettes import BIAS_COLORS, STATUS_COLORS, THRESHOLD_COLORS
...
def select_best(group: pd.DataFrame) -> str:
    ...
    group["norm_jsd_score"] = group["peaks_median_norm_jsd"] - group["peaks_median_jsd"] / 100
    ...
    for tier in ("pass", "warn", "fail"):
        ...
        tied = candidates[
            candidates["norm_jsd_score"] >= best_score * (1 - NORM_JSD_SCORE_TIE_REL_EPS)
        ]
        ...
        idx = near_zero["nonpeaks_pearsonr"].idxmax() if len(near_zero) > 1 else abs_pk_r.idxmin()
...
def generate_explanation(
    ...
    lines = [
        ...
        f"     Ties within {NORM_JSD_SCORE_TIE_REL_EPS:.0%} of the top norm-JSD score are broken by:",
        "       a) preferring pk_r <= np_r (peaks r beating the in-domain nonpeaks fit",
        "          suggests the model learned real peak signal, not just Tn5/AT-content",
        "          bias - see chrombpnet/helpers/generate_reports/make_html_bias.py),",
        "       b) then peaks Pearson r closest to 0,",
        f"       c) unless the closest candidates are within {PEAKS_PEARSONR_NEAR_ZERO_EPS}",
        "          of each other (both close enough to 0), in which case higher",
        "          nonpeaks Pearson r decides instead.",
    ...
    for fold in sorted(selection.index):
        ...
        if status == "PASS":
            lines.append("    Action: Model passes all criteria. Proceed with ChromBPNet training.")
        elif status == "WARN":
...
def plot_boxplot(df: pd.DataFrame, selection: pd.DataFrame, out_stem: Path) -> None:
    ...
    selected_pairs = {
        (str(row["fold"]), row["selected_bias"]) for _, row in selection.reset_index().iterrows()
    }
    ...
    out_data.to_csv(out_stem.parent / (out_stem.name + "_data.tsv"), sep="\t", index=False)
    logger.info(f"Saved: {out_stem}.pdf/.png")
...
def plot_metrics(df: pd.DataFrame, selection: pd.DataFrame, out_stem: Path) -> None:
    ...
    for ax, (metric, ylabel, thresh1, direction, thresh2) in zip(axes, METRIC_CONFIG):
        for i, bias in enumerate(biases):
            vals = [df[(df["bias"] == bias) & (df["fold"] == f)][metric].values for f in folds]
...
# %%
def plot_selection_heatmap(df: pd.DataFrame, selection: pd.DataFrame, out_stem: Path) -> None:
    """
...
# %%
def plot_pearsonr_scatter(df: pd.DataFrame, selection: pd.DataFrame, out_stem: Path) -> None:
    """
    ...
    legend_thresh = [
        mlines.Line2D([0], [0], color="black", lw=0.8, ls="--", label="nonpeaks threshold (0)"),
        mlines.Line2D(
            [0], [0], color=THRESHOLD_COLORS["warn"], lw=0.8, ls="--", label="peaks warn (-0.3)"
        ),
        mlines.Line2D(
            [0], [0], color=THRESHOLD_COLORS["fail"], lw=0.8, ls=":", label="peaks fail (-0.5)"
        ),
    ]
    ax.legend(
    ...
    n = len(df)
    ax.text(0.97, 0.97, f"n={n}", transform=ax.transAxes, fontsize=8, ha="right", va="top")
...
def print_summary(selection: pd.DataFrame) -> None:
    logger.info("\n" + "=" * 70)
    logger.info("BIAS MODEL SELECTION SUMMARY")
    logger.info("=" * 70)
    cols = [
        "selected_bias",
        "status",
    ...
    logger.info(selection[cols].to_string(float_format="{:+.4f}".format))
    logger.info("=" * 70)
...
def main():
    ...
    if overrides:
        logger.info(f"Applying fold-bias overrides from dataset_config.sh: {overrides}")
    ...
    logger.info(f"Tables written to {out_dir}/")

    explanation = generate_explanation(df, selection, args.dataset, args.biases, args.folds)
    (out_dir / "bias_selection_explanation.txt").write_text(explanation)
    logger.info(f"Explanation written to {out_dir}/bias_selection_explanation.txt")
