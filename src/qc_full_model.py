...
from utils.folds import test_chroms
...
apply_style(font_size=14)


# box_with_points() labels axes with real dataset directory names, the same
# keys the cross-dataset plots use, so both share one palette.
...
def box_with_points(ax, labels, group_values, ylabel, title, ylim=None):
    ...
    colors = [DATASET_COLORS.get(lbl, DATASET_COLOR_DEFAULT) for lbl in labels]
...
def plot_cross_dataset_boxplot(combined_df: pd.DataFrame, out_stem: Path) -> None:
    ...
    colors = [DATASET_COLORS_COMBINED.get(d, DATASET_COLOR_DEFAULT) for d in datasets]
    ...
    for ax, (metric, ylabel) in zip(axes, metrics):
        group_values = [
            combined_df[combined_df["dataset"] == d][metric].dropna().values for d in datasets
        ]
    ...
    logger.warning(f"Saved: {out_stem}.pdf/.png")
...
def main():
    ...
    if args.combined:
        ...
        if combined_df.empty:
            ...
            return
        combined_df.to_csv(out_dir / "combined_model_metrics.tsv", sep="\t", index=False)
        plot_cross_dataset_boxplot(combined_df, out_dir / "combined_performance_boxplot")
    ...
    for dataset in datasets_present:
        for fold in args.folds:
            row_match = metrics_df[(metrics_df.dataset == dataset) & (metrics_df.fold == fold)]
            if row_match.empty:
                continue
            for i in range(1, n_motifs + 1):
                col = f"tn5_{i}"
                val = row_match[col].values[0] if col in row_match.columns else float("nan")
                tn5_rows.append(
                    {
                        "dataset": dataset,
                        "fold": fold,
    ...
    if tn5_rows and args.save_plots:
        ...
        fig, axes = plt.subplots(1, n_motifs, figsize=(2.5 * n_motifs, 3.5), sharey=True)
        if n_motifs == 1:
            axes = [axes]

        for ax, motif in zip(axes, [f"tn5_{i}" for i in range(1, n_motifs + 1)]):
            sub = tn5_df[tn5_df.motif == motif]
            vals_by_dataset = [sub[sub.dataset == d]["response"].values for d in datasets_present]
            box_with_points(
                ax,
                datasets_present,
                vals_by_dataset,
