...
def parse_args():
    ...
    p.add_argument("-g", "--genome", type=str, required=True, help="Reference genome FASTA")
    p.add_argument(
        "-c",
        "--chrom-sizes",
        type=str,
...
def main():
    ...
    n_models = len(models)
    logger.info(f"Loaded {n_models} model(s). inputlen={inputlen}, outputlen={outputlen}")
    ...
    bigwig_helper.write_bigwig(avg_profile, regions, gs, bw_out, debug_chr=args.debug_chr)
    logger.info(f"Wrote bigwig: {bw_out}")

    if args.output_bed:
        bed_out = f"{args.output_prefix}_chrombpnet_{args.output_key}_preds_w_logcounts.bed"
        ...
        logger.info(f"Wrote log-counts BED: {bed_out}")
