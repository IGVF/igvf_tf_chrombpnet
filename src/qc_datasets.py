...
from utils import intervals, log
from utils.palettes import DATASET_COLORS_QC as DATASET_COLORS
from utils.palettes import DATASET_LABELS_QC as DATASETS
from utils.palettes import FRAGMENT_SIZE_COLORS
from utils.plotting import apply_style, save_fig
...
def save_dataset_fig(fig, stem):
    """save_fig() rooted at this script's output_path."""
    save_fig(fig, f"{output_path}/{stem}")
...
def compute_frip(fragments_gz, peaks_file):
    """Fragments overlapping at least one peak.

    Replaces `bedtools intersect -a <(zcat ...) -b peaks -u | wc -l`. The `-u`
    counted each fragment once however many peaks it hit, which is what
    intervals.count_in_peaks does; pinned by tests/test_intervals.py.

    The old shell version piped zcat into bedtools without `pipefail`, so a
    truncated fragments file surfaced as a plausible-looking low FRIP rather
    than an error. Reading the file here fails loudly instead.
    """
    frags = pd.read_csv(
        fragments_gz,
        sep="\t",
    ...
    return intervals.count_in_peaks(pr.PyRanges(frags), pr.PyRanges(peaks))
...
for ds_id, ds_label in DATASETS.items():
    ...
    peaks_file = f"{ds_dir}/results/preprocessing/{ds_id}_all_peaks_no_blacklist.narrowPeak"
    ...
    total_frags, frags_per_chrom, n_cells, size_bins = collect_fragment_stats(fragments_gz)

    logger.info("  peaks...")
    total_peaks, peaks_per_chrom, median_peak_width, total_peak_bp = collect_peak_stats(peaks_file)

    logger.info("  FRIP (pyranges)...")
...
for ax, (col, title, scale, unit) in zip(axes, METRICS):
    ...
    bars = ax.bar(range(len(ds_order)), vals, color=colors, width=0.6, edgecolor="white")
...
plt.tight_layout()
if save_plots:
    save_dataset_fig(fig, "summary_metrics")
...
plt.tight_layout()
if save_plots:
    save_dataset_fig(fig, "peaks_per_chromosome")
...
plt.tight_layout()
if save_plots:
    save_dataset_fig(fig, "fragments_per_chromosome_norm")
...
size_colors = FRAGMENT_SIZE_COLORS
...
plt.tight_layout()
if save_plots:
    save_dataset_fig(fig, "fragment_size_distribution")
...
lines += [
    ...
    "  2. Update 00.0.prepare_signal.sh for merged fragments and peaks.",
